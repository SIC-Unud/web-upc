...
<div x-data="{modalOpen: false, formOpen: false, modalContent: '', modalTitle: '', isMobile: window.innerWidth < 768, editMode: false}" x-init="$watch('isMobile', value => {}); window.addEventListener('resize', () => isMobile = window.innerWidth < 768)" class="px-4 relative font-jakarta">
    ...
    <div class="flex justify-between items-center py-8">
        ...
        <button @click="formOpen = true; modalTitle=''; modalContent=''; editMode=false" class="bg-[#007BFF] hover:shadow-xl text-white font-bold py-2 px-4 text-xs md:text-base">
            + Tambah
        </button>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="fixed inset-0 flex items-center justify-center backdrop-blur-sm bg-black/10 z-50" x-show="modalOpen" x-transition>
            <div class="bg-white w-[70%] md:w-[60%] h-[75%] p-6 rounded-xl shadow-xl overflow-hidden">
                <div class="flex justify-between items-start mb-3">
                    <h2 class="font-bold text-lg md:text-2xl text-black mb-3" x-text="modalTitle"></h2>
                ...
                <form @submit.prevent class="space-y-4">
                    <div>
                        <input type="text" name="title"
                            class="mt-1 block w-full rounded-md border-black shadow-sm p-3 font-bold text-lg md:text-2xl text-black"
                            placeholder="Masukkan judul..."
                            x-model="modalTitle">
                    </div>
                    <div>
                        <textarea name="content" rows="4"
                            class="mt-1 block w-full rounded-md text-sm md:text-lg text-black text-justify overflow-y-auto min-h-[210px] p-3 pr-5"
                            placeholder="Tulis isi informasi..."
                            x-model="modalContent"></textarea>
                    </div>
                    <div class="text-right">
                        <button type="button" class="bg-[#029161] hover:shadow-xl text-white font-bold py-2 px-6"
