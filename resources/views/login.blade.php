         ...
         <form  action="{{ route('login.post') }}" method="POST" class="mb-2">
            ...
            <div>
               <div class="border relative border-white bg-black/70 w-fit px-2 pb-7 pt-3 max-w-full my-4 mx-12 md:w-[525px] md:px-5 md:py-8">
                  ...
                  <div x-data="{ showPassword: false }" class="relative w-full mt-5">
                     <input 
                        :type="showPassword ? 'text' : 'password'" 
                        name="password" 
                        placeholder="Password"
                        class="border bg-gray-100 rounded-none text-sm md:text-base w-full h-9 md:h-12 p-2 pr-10 z-10">
                     <button 
                        type="button" 
                        @click="showPassword = !showPassword"
                        class="absolute inset-y-0 right-0 flex items-center px-3 text-gray-500 z-20">
